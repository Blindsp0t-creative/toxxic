...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    #if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
        rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * float4(0.08393916,0.02025841,0.02025841,1);
    #else
        rimColor = float4(0.08393916,0.02025841,0.02025841,1);
    #endif
    ...
    rim *= float4(0.08393916,0.02025841,0.02025841,1).a * rimColor.a * rimMask;
    rimLightEmission = rim * lerp(albedo, rimColor, float(0.564)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, float(0.564)) + lerp(albedo.rgb, rimColor, float(0.564)) * rimBrighten, rim);
}
#endif
