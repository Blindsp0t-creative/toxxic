    ...
    void applyGlitter(inout float4 albedo, inout float3 glitterEmission)
    {
        ...
        switch(float(0))
        {
            ...
            finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.564)), poiCam.viewDir)), float(300)), 0);
        ...
        if(float(0) == 1)
        {
            albedo.rgb = lerp(albedo.rgb, finalGlitter * glitterColor * float(0.5), finalGlitter * glitterTexture.a * glitterMask);
            glitterEmission = finalGlitter * glitterColor * max(0, (float(0.5) - 1) * glitterTexture.a) * glitterMask;
        }
        else
        {
            glitterEmission = finalGlitter * glitterColor * float(0.5) * glitterTexture.a * glitterMask;
        }
    }
