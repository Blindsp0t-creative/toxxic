    ...
    void applyGlitter(inout float4 albedo, inout float3 glitterEmission)
    {
        ...
        // Scale
        float2 st = frac(poiMesh.uv[0] + float4(0,0,0,0).xy * _Time.x) * float(1000);
        ...
        switch(float(0))
        {
            ...
            if(float(2) > 0)
            {
                randomRotation = randomFloat3WiggleRange(randoPoint, float(39.9));
            }
            else
            {
                randomRotation = randomFloat3Range(randoPoint, float(39.9));
            }
            ...
            finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.8)), poiCam.viewDir)), float(248)), 0);
        ...
        glitterColor *= lerp(1, albedo, float(0.5));
        #if defined(PROP_GLITTERCOLORMAP) || !defined(OPTIMIZER_ENABLED)
