    ...
    void applyGlitter(inout float4 albedo, inout float3 glitterEmission)
    {
        ...
        switch(float(0))
        {
            ...
            if(float(10) > 0)
            {
                randomRotation = randomFloat3WiggleRange(randoPoint, float(42.8));
            }
            else
            {
                randomRotation = randomFloat3Range(randoPoint, float(42.8));
            }
            ...
            finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.486)), poiCam.viewDir)), float(300)), 0);
        ...
        if(float(0) == 1)
        {
            albedo.rgb = lerp(albedo.rgb, finalGlitter * glitterColor * float(1), finalGlitter * glitterTexture.a * glitterMask);
            glitterEmission = finalGlitter * glitterColor * max(0, (float(1) - 1) * glitterTexture.a) * glitterMask;
        }
        else
        {
            glitterEmission = finalGlitter * glitterColor * float(1) * glitterTexture.a * glitterMask;
        }
    }
