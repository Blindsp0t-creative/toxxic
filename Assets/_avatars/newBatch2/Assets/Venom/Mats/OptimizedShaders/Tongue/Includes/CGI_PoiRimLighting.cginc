...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    float rimWidth = lerp( - .05, 1, float(0.16));
    #ifdef POI_AUDIOLINK
    ...
    rimLightEmission = rim * lerp(albedo, rimColor, float(0.05)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, float(0.05)) + lerp(albedo.rgb, rimColor, float(0.05)) * rimBrighten, rim);
}
#endif
