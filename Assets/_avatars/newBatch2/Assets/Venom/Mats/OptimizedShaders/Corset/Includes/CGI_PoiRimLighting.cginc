...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    float rimWidth = lerp( - .05, 1, float(0.265));
    #ifdef POI_AUDIOLINK
    ...
    rimLightEmission = rim * lerp(albedo, rimColor, float(0.016)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, float(0.016)) + lerp(albedo.rgb, rimColor, float(0.016)) * rimBrighten, rim);
}
#endif
