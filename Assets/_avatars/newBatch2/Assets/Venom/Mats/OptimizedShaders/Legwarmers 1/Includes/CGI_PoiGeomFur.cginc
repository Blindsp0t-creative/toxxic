...
void geomFur(triangle v2f IN[3], inout TriangleStream < v2f > tristream, uint InstanceID : SV_GSInstanceID)
{
	...
	v2f o_fur[3];
	float3 offset = (float3(0, -1, 0) * (float(0.075)) *float(1));
	...
	for (int i = 0; i < 3; i++)
	{
		...
		o_fur[i] = IN[i];
		fur_worldPos[i] = float4(IN[i].worldPos + IN[i].normal * float(0.075), 1);
	}
	if (InstanceID == 0) {
		tristream.RestartStrip();
	}
	int Total_FurLayers = clamp(floor(float(23) * (1 - smoothstep(float(0), float(25), distance(IN[0].worldPos, getCameraPosition())))), min(1, float(23)), float(23));
