...
void applyDecals(inout float4 albedo, inout float3 decalEmission)
{
    ...
    decalScale = float4(1,1,0,0);
        ...
        #endif
        uv = decalUV(float(0), float4(0.5,0.5,0,0), float(0), float(0), decalScale);
    ...
    albedo.rgb = lerp(albedo.rgb, customBlend(albedo.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(float(1) + lerp(float4(0,0,0,0).x, float4(0,0,0,0).y, poiMods.audioLink[float(0)])));
    
    decalEmission += decalColor.rgb * decalColor.a * max(float(0) + lerp(float4(0,0,0,0).x, float4(0,0,0,0).y, poiMods.audioLink[float(0)]), 0);
    #ifdef GEOM_TYPE_BRANCH_DETAIL
        // Decal 1
        decalScale = float4(1,1,0,0);
        #if defined(PROP_DECALTEXTURE1) || !defined(OPTIMIZER_ENABLED)
