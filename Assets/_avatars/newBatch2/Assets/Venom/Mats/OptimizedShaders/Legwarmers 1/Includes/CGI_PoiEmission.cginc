...
float3 calculateEmissionNew(in float4 baseColor, inout float4 finalColor)
{
    ...
    float3 emissionColor0 = 0;
        ...
        if (float(0))
        {
            emissionStrength0 *= getBandAtTime(float(0), saturate(1 - poiLight.nDotV));
        }
    ...
    float3 emissionColor1 = 0;
        ...
        emissionStrength1 = float(0);
            ...
            if (float(0))
            {
                emissionStrength1 *= getBandAtTime(float(0), saturate(1 - poiLight.nDotV));
            }
            emissionStrength1 += lerp(float4(0,0,-10,10).x, float4(0,0,-10,10).y, poiMods.audioLink[float(0)]);
