...
//global
	...
	float3 calculateNormal(float3 baseNormal)
	{
		...
		float3 normal = UnpackScaleNormal(UNITY_SAMPLE_TEX2D_SAMPLER(_IridescenceNormalMap, _MainTex, TRANSFORM_TEX(poiMesh.uv[float(0)], _IridescenceNormalMap)), float(0));
		return normalize(
...
void applyIridescence(inout float4 albedo, inout float3 IridescenceEmission)
{
	...
	albedo.rgb = lerp(albedo.rgb, saturate(iridescenceColor.rgb * float(1.75)), iridescenceColor.a * float(0) * iridescenceMask);
	albedo.rgb += saturate(iridescenceColor.rgb * float(1.75) * iridescenceColor.a * float(0.454) * iridescenceMask);
	albedo.rgb *= saturate(lerp(1, iridescenceColor.rgb * float(1.75), iridescenceColor.a * float(0) * iridescenceMask));
	...
	IridescenceEmission = saturate(iridescenceColor.rgb * float(1.75)) * iridescenceColor.a * iridescenceMask * emissionStrength;
}
