    ...
    void applyGlitter(inout float4 albedo, inout float3 glitterEmission)
    {
        ...
        for (int j = -1; j <= 1; j ++)
        {
            for (int i = -1; i <= 1; i ++)
            {
                ...
                float2 rando = pos;
                pos = 0.5 + 0.5 * sin(float(2) * 6.2831 * pos);
        ...
        switch(float(1))
        {
            ...
            finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.8)), poiCam.viewDir)), float(0)), 0);
            finalGlitter *= glitterAlpha;
            break;
            case 1:
            float offset = random(randoPoint);
            float brightness = sin((_Time.x + offset) * float(10)) * float(75.1) - (float(75.1) - 1);
