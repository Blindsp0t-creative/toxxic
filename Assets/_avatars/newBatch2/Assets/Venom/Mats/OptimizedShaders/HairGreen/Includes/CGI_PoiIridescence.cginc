...
void applyIridescence(inout float4 albedo, inout float3 IridescenceEmission)
{
	...
	albedo.rgb = lerp(albedo.rgb, saturate(iridescenceColor.rgb * float(2)), iridescenceColor.a * float(0) * iridescenceMask);
	albedo.rgb += saturate(iridescenceColor.rgb * float(2) * iridescenceColor.a * float(0.454) * iridescenceMask);
	albedo.rgb *= saturate(lerp(1, iridescenceColor.rgb * float(2), iridescenceColor.a * float(0) * iridescenceMask));
	...
	IridescenceEmission = saturate(iridescenceColor.rgb * float(2)) * iridescenceColor.a * iridescenceMask * emissionStrength;
}
