Shader "Hidden/Locked/.poiyomi/Old Versions/8.1/Poiyomi Toon/2da1b2076d27ab5418a3718a9717a1bb"
{
	Properties
	{
		...
		_DissolveDetailEdgeSmoothing_Metal ("Detail Noise Smoothing", Range(0, 1)) = 0
		_DissolveDetailStrength_Metal ("Dissolve Detail Strength", Range(0, 1)) = 0.1
 ...
 #define OPTIMIZER_ENABLED 
			...
			float _DissolveDetailStrength_Metal;
			float _DissolveDetailEdgeSmoothing_Metal;
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods, in PoiCam poiCam, in PoiLight poiLight)
			{
				...
				#if defined(PROP_DISSOLVETOTEXTURE) || !defined(OPTIMIZER_ENABLED)
				dissolveToTexture = POI2D_SAMPLER_PAN(_DissolveToTexture, _MainTex, poiUV(poiMesh.uv[(0.0 /*_DissolveToTextureUV*/)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0,0.7687256,1,0).rgb, (0.0 /*_DissolveTextureColorThemeIndex*/)), float4(0,0.7687256,1,0).a);
				#else
				...
				+ (0.0 /*_DissolveAlpha8*/)
				+ (0.0 /*_DissolveAlpha9*/);
				float dds = _DissolveDetailStrength_Metal;
				...
				switch((1.0 /*_DissolveType*/))
				{
					default: // Basic (case 1)
					{
						...
						dissolveAlpha = da;
						dds *= smoothstep(1, 0.99, da) * lerp(1, smoothstep(0, lerp(0.01, 0.1, dds), da), _DissolveDetailEdgeSmoothing_Metal);
					...
					case 3: // Spherical
					{
						...
						dissolveAlpha = da;
						dds *= smoothstep(0, 0.2*dds + 0.01, dissolveAlpha) * lerp(1, smoothstep(1, 1 - 0.2*dds - 0.01, dissolveAlpha), _DissolveDetailEdgeSmoothing_Metal);
			...
			void applyCubemap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				cubeMap.rgb *= poiThemeColor(poiMods, float4(0.1499598,0.1499598,0.1499598,1), (0.0 /*_CubeMapColorThemeIndex*/));
				#else
				float4 cubeMap = float4(0.21763764082, 0.21763764082, 0.21763764082, .5) * float4(poiThemeColor(poiMods, float4(0.1499598,0.1499598,0.1499598,1), (0.0 /*_CubeMapColorThemeIndex*/)), 1);
				#endif
				...
				poiFragData.emission += cubeMap.rgb * (0.74 /*_CubeMapEmissionStrength*/) * CubeMapMask * cubeMap.a;
			}
			#endif
